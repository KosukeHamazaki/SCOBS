#####################################################################################################################################################
######  Title: 3.2_SimulatedCrop_SCOBS_simulate_breeding_scheme_via_genomic_selection_with_simply_optimized_selection_index_check_convergence  ######
######  Author: Kosuke Hamazaki (hamazaki@ut-biomet.org)                                                                                       ######
######  Affiliation: Lab. of Biometry and Bioinformatics, The University of Tokyo                                                              ######
######  Date: 2021/02/18 (Created), 2021/02/18 (Last Updated)                                                                                  ######
#####################################################################################################################################################
...
for (simGenoNo in 1:nSimGeno) {
  ...
  for (simPhenoNo in 1:nSimPheno) {
    ...
    for (strategyName in strategyNames) {
      ...
      fileNameSimBsBVNow <- paste0(dirMidSCOBSBV, scriptIDMainOpt, "_", simBsName, "_simBs.rds")

      if (file.exists(fileNameSimBsBVNow)) {

        ###### 3. Simulate breeding scheme using simple GS with each BV ######
        ##### 3.0. Load optimized simBsOpt obkect and extract optimized hyperparameters #####
        mySimBsBVOptNow <- readRDS(file = fileNameSimBsBVNow)
        optimalHyperParamMat <- mySimBsBVOptNow$optimalHyperParamMatsList[[1]]
        optimalHyperParamMat <- rbind(Iteration_0 = rep(0.5, ncol(optimalHyperParamMat)),
                                      optimalHyperParamMat)

        optimalHyperParamMatDup <- duplicated(x = optimalHyperParamMat)
        optimalHyperParamMatID <- cumsum(!optimalHyperParamMatDup)
        optimalHyperParamMatUniq <- unique(x = optimalHyperParamMat)
        uniqIterationNames <- rownames(optimalHyperParamMatUniq)
        cat(paste0("Re-evaluate objective function for:\n",
                   paste(uniqIterationNames, collapse = ", "),
                   "\n"))
        nUniq <- nrow(optimalHyperParamMatUniq)

        # mySimBsBVConvList <- list()
        # uniqNo <- 1

        for (uniqNo in 1:nUniq) {
          st <- Sys.time()
          uniqIterationName <- uniqIterationNames[uniqNo]
          print(uniqIterationName)
          simBsNameConv <- paste0(simBsName, "-", uniqIterationName)
          fileNameSimBsBVConvNow <- paste0(dirMidSCOBSBV, scriptID, "_", simBsNameConv, "_simBs.rds")
          uniqOptParamNow <- optimalHyperParamMatUniq[uniqNo, ]
          hListNow <- split(x = uniqOptParamNow * (mySimBsBVOptNow$hMax - mySimBsBVOptNow$hMin) +
                              mySimBsBVOptNow$hMin,
                            f = rep(1:mySimBsBVOptNow$nGenerationProceedSimulation,
                                    mySimBsBVOptNow$hLens))
          ...
          saveRDS(object = mySimBsBVNow, file = fileNameSimBsBVConvNow)
          # mySimBsBVConvList[[simBsNameConv]] <- mySimBsBVNow
        ...
        ##### 3.3. Set simBs class object for equal allocation #####
        simBsNameEqual <- paste0(simBsName, "-equal_allocation")
        ...
        ##### 3.4. Start simulation, summary results, and save simBs class object for equal allocation #####
        print("Start simulation....")
        ...
        fileNameSimBsEqual <-  paste0(dirMidSCOBSBV, scriptID, "_", simBsNameEqual, "_simBs.rds")
        saveRDS(object = mySimBsBVEqual, file = fileNameSimBsEqual)
        # mySimBsBVConvList[[simBsNameEqual]] <- mySimBsBVEqual
    ...
    for (strategyName in strategyNames) {
      ...
      if (file.exists(fileNameSimBsBVNow)) {
        ...
        for (uniqNo in 1:nUniq) {
          ...
          ##### 3.1. Set simBs class object #####
          mySimBsBVNow <- readRDS(file = fileNameSimBsBVConvNow)
          mySimBsBVConvList[[simBsNameConv]] <- mySimBsBVNow
        ...
        mySimEval <- myBreedSimulatR::simEval$new(simEvalName = simBsName,
                                                  simBsList = mySimBsBVConvList,
                                                  verbose = verbose)

        fileNameSimEval <- paste0(dirMidSCOBSBV, scriptID, "_", simBsName,
        ...
        optimalObjectives <- (trueGVSummaryArrayForConv[1, 5, 1:nUniq])[optimalHyperParamMatID]
        names(optimalObjectives) <- rownames(optimalHyperParamMat)

        dfForGGPlot <- data.frame(Iteration = readr::parse_number(names(optimalObjectives)),
                                  Value = optimalObjectives)

        pltConv <- ggplot2::ggplot(data = dfForGGPlot) +
          ggplot2::geom_line(mapping = aes(x = Iteration,
                                           y = Value)) +
          ggplot2::geom_hline(yintercept = trueGVSummaryArrayForConv[1, 5, nUniq + 1],
