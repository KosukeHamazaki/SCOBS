#############################################################################################################################################
######  Title: 3.1_SimulatedCrop_SCOBS_summary_and_compare_breeding_schemes_based_on_genomic_selection_by_diiferent_selection_indices  ######
  ...
  for (simPhenoNo in 1:nSimPheno) {
    ...
    for (trainingPopInitNo in 1:length(nTrainingPopInits)) {
      ...
      for (strategyName in strategyNames) {
        ...
        dirMidSCOBSBV <- paste0(dirMidSCOBSTrainingPop, scriptIDMain, "_", simBsName, "/")
        if (!dir.exists(dirMidSCOBSBV)) {
          dir.create(dirMidSCOBSBV)
        }

        # if ((simGenoNo == 1) & (simPhenoNo == 1)) {
        #   saveAllResAt <- paste0(dirMidSCOBSBV, scriptIDMain, "_", saveAllResAtBase)
        # } else {
        #   saveAllResAt <- NULL
        # }
        saveAllResAt <- NULL
        # summaryAllResAt <- paste0(dirMidSCOBSBV, scriptIDMain, "_", summaryAllResAtBase)
        summaryAllResAt <- NULL


        fileNameSimBsBVNow <- paste0(dirMidSCOBSBV, scriptIDMain, "_", simBsName, "_simBs.rds")
    ...
    fileNameSimEval <- paste0(dirMidSCOBSPheno, scriptID, "_", simBsNamePheno, "_simEval.rds")
